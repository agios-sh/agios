...
func TestCacheCorrupted(t *testing.T) {
	...
	dir := filepath.Join(tmp, agiosDir)
...
func TestAssetName(t *testing.T) {
	name := AssetName()
	if runtime.GOOS == "windows" {
		if name != "agios_windows_"+runtime.GOARCH+".zip" {
			t.Errorf("AssetName() = %q, unexpected", name)
		}
	} else {
		expected := "agios_" + runtime.GOOS + "_" + runtime.GOARCH + ".tar.gz"
		if name != expected {
...
func TestExtractTarGz(t *testing.T) {
	// Create a tar.gz with an "agios" binary inside
...
func TestExtractZip(t *testing.T) {
	// Create a zip with an "agios.exe" binary inside
...
// createTestTarGz builds a tar.gz archive containing a fake "agios" binary
...
func TestApplyE2E(t *testing.T) {
	...
	targetDir := t.TempDir()
	targetPath := filepath.Join(targetDir, "agios")
	...
	// We can't do that directly, so instead test the individual steps.
	// Download
